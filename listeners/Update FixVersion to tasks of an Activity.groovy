...
\
logger.info("Starting Activity Update Listener.")\
\
...
if (activity.getIssueType().getName() == "Activity") \{\
    issueLinkManager.getOutwardLinks(activity.id).each \{taskLink ->\
        if ( taskLink.getIssueLinkType().getName() == "Parent/Child" ) \{\
            ...
            \
            Version[] activityVersions = activity.getFixVersions()\
            def resultVersions = issue.getFixVersions()            \
            \
            activityVersions.each \{ Version activityVersion->\
                projectVersions.each \{ Version projectVersion->\
\
                    if ( activityVersion.getName() == projectVersion.getName() ) \{\
