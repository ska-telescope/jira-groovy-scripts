...
\
logger.info("Starting Activity Update Listener to copy Workstream and TDT Epic to Task")\
\
IssueManager issueManager = ComponentAccessor.getIssueManager();\
IssueLinkManager issueLinkManager = ComponentAccessor.getIssueLinkManager();\
CustomFieldManager customFieldManager = ComponentAccessor.getCustomFieldManager()\
Issue activity = event.issue\
Issue issue\
\
if (activity.getIssueType().getName() == "Activity") \{\
    issueLinkManager.getOutwardLinks(activity.id).each \{taskLink ->\
  ...
  \
				...
				def workstreamValue = activity.getCustomFieldValue(workstreamField)\
                \
                def tdtEpicField = customFieldManager.getCustomFieldObject(12204) // "TDT Epic"\
				def tdtEpicValue = activity.getCustomFieldValue(tdtEpicField)\
                \
