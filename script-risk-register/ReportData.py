...
def analyze(issues,issues_mit,jira,days):
	...
	table_7 = pd.concat([table_7,table_6.sum(numeric_only=True)], ignore_index=True)
	...
	shutil.copyfile('R&O Report Template.xlsx',filename)
	reader = pd.read_excel(filename)
	book = openpyxl.load_workbook(filename)
	with pd.ExcelWriter(filename, datetime_format='YYYY-MM-DD HH:MM:SS', engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
